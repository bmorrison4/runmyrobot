...
parser.add_argument('--table', help="enable table top mode",
                    dest='table', action='store_true')
parser.set_defaults(table=False)
parser.add_argument(
    '--info-server', help="Server that robot will connect to for information about servers and things", default='letsrobot.tv')
parser.add_argument(
    '--type', help="Serial or motor_hat or gopigo2 or gopigo3 or l298n or motozero or pololu or mdd10", default='motor_hat')
parser.add_argument('--serial-device', help="Serial device",
                    default='/dev/ttyACM0')
...
parser.add_argument(
    '--led', help="Type of LED for example max7219", default=None)
parser.add_argument(
    '--ledrotate', help="Rotates the LED matrix. Example: 180", default=None)
...
parser.add_argument('--no-chat-server-connection',
                    dest='enable_chat_server_connection', action='store_false')
parser.set_defaults(enable_chat_server_connection=True)
parser.add_argument('--no-secure-cert', dest='secure_cert',
                    action='store_false')
parser.set_defaults(secure_cert=True)
parser.add_argument('--filter-url-tts',
                    dest='filter_url_tts', action='store_true')
parser.set_defaults(filter_url_tts=False)
parser.add_argument('--slow-for-low-battery',
                    dest='slow_for_low_battery', action='store_true')
...
parser.add_argument('--tts-delay-enabled',
                    dest='tts_delay_enabled', action='store_true')
parser.add_argument('--tts-delay-seconds',
                    dest='tts_delay_seconds', type=int, default=5)
parser.add_argument(
    '--woot-room', help="Room to enable woot events", default='')
parser.add_argument('--google-tts', help='Enable Google TTS',
                    dest='google_tts', action='store_true')
parser.set_defaults(google_tts=False)
parser.add_argument('--google-auth-key',
                    help='Authentication key for Google TTS', default='/home/pi/key.json')
parser.add_argument(
    '--google-voice', help='Name of the voice for Google TTS', default='en-US-Standard-A')
parser.add_argument('--google-language-code',
                    help='Language code for Google TTS', default='en-US')
...
os.system("amixer -c %d cset numid=3 %d%%" %
          (commandArgs.speaker_device, commandArgs.tts_volume))
...
if commandArgs.type == 'l298n':
    ...
    if str(robotID) in gpio_config.sections():
        ...
        config_id = 'default'
    StepPinForward = int(str(gpio_config[config_id]['StepPinForward']).split(
        ',')[0]), int(str(gpio_config[config_id]['StepPinForward']).split(',')[1])
    StepPinBackward = int(str(gpio_config[config_id]['StepPinBackward']).split(
        ',')[0]), int(str(gpio_config[config_id]['StepPinBackward']).split(',')[1])
    StepPinLeft = int(str(gpio_config[config_id]['StepPinLeft']).split(
        ',')[0]), int(str(gpio_config[config_id]['StepPinLeft']).split(',')[1])
    StepPinRight = int(str(gpio_config[config_id]['StepPinRight']).split(
        ',')[0]), int(str(gpio_config[config_id]['StepPinRight']).split(',')[1])
...
def getChatHostPort():
    url = 'https://%s/get_chat_host_port/%s' % (
        infoServer, commandArgs.robot_id)
    response = robot_util.getWithRetry(url, secure=commandArgs.secure_cert)
    return json.loads(response)
...
controlSocketIO = SocketIO(
    controlHostPort['host'], controlHostPort['port'], LoggingNamespace, transports='websocket')
...
if commandArgs.enable_chat_server_connection:
    ...
    chatSocket = SocketIO(
        chatHostPort['host'], chatHostPort['port'], LoggingNamespace, transports='websocket')
...
if commandArgs.tts_delay_enabled or commandArgs.woot_room != '':
    userSocket = SocketIO('https://letsrobot.tv', 8000,
                          LoggingNamespace, transports='websocket')

print("connecting to app server socket.io")
appServerSocketIO = SocketIO(
    infoServer, 8022, LoggingNamespace, transports='websocket')
...
def configWifiLogin(secretKey):
    ...
    try:
        ...
        wpaText = WPA_FILE_TEMPLATE.format(
            name=responseJson['wifi_name'], password=responseJson['wifi_password'])
...
def say(message):
    ...
    #os.system('espeak < /tmp/speech.txt')
        ...
        for hardwareNumber in (2, 0, 3, 1, 4):
            ...
            if commandArgs.male:
                os.system('cat ' + tempFilePath +
                          ' | espeak --stdout | aplay -D plughw:%d,0' % hardwareNumber)
            else:
...
def handle_user_socket_chat_message(args):
    if args['name'] == 'LetsBot' and args['room'] == commandArgs.woot_room:
        ...
        if len(messageSplit) == 8:
            # yes.
            if messageSplit[1] == 'wooted' and messageSplit[3] == 'robits' and messageSplit[4] == 'to' and messageSplit[6] == '!!' and messageSplit[7] == '':
...
def handleStartReverseSshProcess(args):
    ...
    appServerSocketIO.emit(
        "reverse_ssh_info", "return code: " + str(returnCode))
...
def onHandleControlDisconnect(*args):
    ...
    print()
    # Reget control port will start if it closed for whatever reason
    newControlHostPort = getControlHostPort()
    # See if the port is not the same as before
    if controlHostPort['port'] != newControlHostPort['port']:
        print("restart: control host port changed")
        # Auto restart script will restart if the control port is not the same (which is unlikely)
        sys.exit(1)
...
def isCharging():
    ...
    if commandArgs.type == "motor_hat":
        ...
        if chargeValue < 99:  # if it's not full charged already
            ...
            # return whether it's connected to the dock
...
def setSpeedBasedOnCharge():
    ...
    if chargeValue < 30:
        ...
        dayTimeDrivingSpeedActuallyUsed = int(
            float(commandArgs.day_speed) * multiple)
        nightTimeDrivingSpeedActuallyUsed = int(
            float(commandArgs.night_speed) * multiple)
    else:
        dayTimeDrivingSpeedActuallyUsed = commandArgs.day_speed
        nightTimeDrivingSpeedActuallyUsed = commandArgs.night_speed
